...
function exportCSV() {
  const headers = ['Hari', 'Air Dihemat (L)', 'Pupuk Dihemat (kg)', 'Biaya Dihemat (Rp)', 'Energi Dihemat (kWh)'];
...
function exportPDF() {
  // Uses browser native print to PDF — no extra library needed
  const printContent = `
    <!DOCTYPE html>
    <html lang="id">
    <head>
    ...
    <body>
      <h1>🌱 NutriGrow — Laporan Eco-Savings</h1>
      <h2>Periode: ${new Date().toLocaleDateString('id-ID', { year: 'numeric', month: 'long' })} | Dicetak: ${new Date().toLocaleString('id-ID')}</h2>

      <div class="kpi-grid">
        <div class="kpi-card">
          <div class="kpi-value">${formatNumber(mockEcoSavings.water_saved_liters)} L</div>
          <div class="kpi-label">💧 Air Dihemat</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-value">${formatNumber(mockEcoSavings.fertilizer_saved_kg)} kg</div>
          <div class="kpi-label">🌿 Pupuk Dihemat</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-value">${formatCurrency(mockEcoSavings.cost_saved_rupiah)}</div>
          <div class="kpi-label">💰 Biaya Dihemat</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-value">${formatNumber(mockEcoSavings.energy_saved_kwh)} kWh</div>
          <div class="kpi-label">⚡ Energi Dihemat</div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <BigKPICard icon={Droplets}   label="Air Dihemat"    value={mockEcoSavings.water_saved_liters}  unit="L"   trend={mockEcoSavings.water_trend}       bgGradient="bg-gradient-to-br from-secondary-400 to-secondary-600" delay={0} />
        <BigKPICard icon={Leaf}       label="Pupuk Dihemat"  value={mockEcoSavings.fertilizer_saved_kg} unit="kg"  trend={mockEcoSavings.fertilizer_trend}   bgGradient="bg-gradient-to-br from-primary-400 to-primary-600" delay={100} />
        <BigKPICard icon={DollarSign} label="Biaya Dihemat"  value={mockEcoSavings.cost_saved_rupiah}   unit="Rp"  trend={mockEcoSavings.cost_trend}         bgGradient="bg-gradient-to-br from-accent-400 to-accent-600" delay={200} />
        <BigKPICard icon={Zap}        label="Energi Dihemat" value={mockEcoSavings.energy_saved_kwh}    unit="kWh" trend={mockEcoSavings.energy_trend}       bgGradient="bg-gradient-to-br from-purple-400 to-purple-600" delay={300} />
      </div>

      {/* Charts */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="glass p-5 opacity-0 animate-fade-in-up" style={{ animationDelay: '400ms', animationFillMode: 'forwards' }}>
          <h3 className="text-base font-semibold mb-4" style={{ color: 'var(--surface-text)' }}>💧 Penghematan Air &amp; Pupuk</h3>
        ...
        <div className="glass p-5 opacity-0 animate-fade-in-up" style={{ animationDelay: '500ms', animationFillMode: 'forwards' }}>
          <h3 className="text-base font-semibold mb-4" style={{ color: 'var(--surface-text)' }}>💰 Tren Penghematan Biaya &amp; Energi</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="glass p-5 opacity-0 animate-fade-in-up" style={{ animationDelay: '600ms', animationFillMode: 'forwards' }}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-semibold" style={{ color: 'var(--surface-text)' }}>📊 Detail Penghematan Mingguan</h3>
