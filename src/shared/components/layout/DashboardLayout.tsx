    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside
        className={cn(
          ...
          'bg-gradient-to-b from-primary-900 via-primary-800 to-primary-950 text-white',
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 h-16 border-b border-white/10 shrink-0">
          ...
          {!collapsed && (
            <div className="animate-fade-in">
              <h1 className="font-bold text-base tracking-tight font-[var(--font-display)]">{APP_NAME}</h1>
              <p className="text-[10px] text-primary-300 -mt-0.5">Smart Fertigation</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
          {visibleNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={cn('w-5 h-5 shrink-0', isActive && 'text-primary-300')} />
                {!collapsed && (
                  <span className="text-sm font-medium truncate">{item.label}</span>
                )}
                {item.id === 'notifications' && unreadCount > 0 && (
                  <span className={cn(
                    'absolute bg-danger-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center',
        ...
        <div className="hidden lg:block border-t border-white/10 p-2">
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-primary-300 hover:bg-white/8 hover:text-white transition-colors"
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Topbar */}
        <header className="h-16 shrink-0 border-b flex items-center justify-between px-4 lg:px-6 glass-sm" style={{ borderColor: 'var(--surface-border)' }}>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold" style={{ color: 'var(--surface-text)' }}>
                {navItems.find(n => n.href === pathname)?.label || 'Dashboard'}
              </h2>
              <p className="text-xs hidden sm:block" style={{ color: 'var(--surface-text-muted)' }}>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Notification bell */}
            <Link href="/notifications" className="p-2 rounded-lg transition-colors hover:bg-primary-50 relative" style={{ color: 'var(--surface-text-muted)' }}>
            ...
            {/* User + Logout */}
            <div className="hidden sm:flex items-center gap-2 pl-2 ml-1 border-l" style={{ borderColor: 'var(--surface-border)' }}>
              {profile?.avatar_url ? (
                <img
                  src={profile.avatar_url}
                  alt={profile.full_name || 'User'}
                  className="w-8 h-8 rounded-full object-cover ring-2 ring-primary-200 shadow-sm"
              ...
              <div className="hidden md:block">
                <p className="text-sm font-medium" style={{ color: 'var(--surface-text)' }}>{profile?.full_name || 'Pengguna'}</p>
              ...
              <button
                ...
                className="ml-1 p-2 rounded-lg transition-colors hover:bg-danger-50 hover:text-danger-600 disabled:opacity-50"
                style={{ color: 'var(--surface-text-muted)' }}
                title="Keluar"
              >
